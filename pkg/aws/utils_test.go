...
var payloadAWSActions = []payloadAWSAction{
	...
	// ec2:CreateSnapshot is unscoped: aws:ResourceTag is evaluated against the
	// new snapshot resource, which has no tags at IAM authorization time.
	{"ec2:CreateSnapshot", false},
	{"ec2:CreateTags", false},
	{"ec2:CreateVolume", false},
	// ec2:DeleteSnapshot is unscoped for the same reason as CreateSnapshot.
	...
	// ec2:ModifyNetworkInterfaceAttribute is unscoped: AWS evaluates the
	// aws:ResourceTag condition against referenced security groups, which may
	// not carry the kubernetes.io/cluster/ ownership tag.
...
func TestSupportsInfraResourceTagCondition(t *testing.T) {
	...
	t.Run("snapshot create is unscoped", func(t *testing.T) {
		supported, err := SupportsInfraResourceTagCondition("ec2:CreateSnapshot")
		require.NoError(t, err)
		assert.False(t, supported)
	})

	t.Run("snapshot delete is unscoped", func(t *testing.T) {
		supported, err := SupportsInfraResourceTagCondition("ec2:DeleteSnapshot")
		require.NoError(t, err)
		assert.False(t, supported)
	})

	t.Run("ModifyNetworkInterfaceAttribute is unscoped", func(t *testing.T) {
		supported, err := SupportsInfraResourceTagCondition("ec2:ModifyNetworkInterfaceAttribute")
		require.NoError(t, err)
		assert.False(t, supported)
	})
